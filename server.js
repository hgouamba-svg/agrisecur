...
router.post("/api/sellers/me/kyc-documents", (req, res, params, body) => {
  ...
  // Notification interne — envoyée en tâche de fond (fire-and-forget) pour
  // ne jamais retarder la réponse au vendeur ni faire échouer sa soumission
  // si l'envoi d'email rencontre un problème (SMTP indisponible, etc.).
  const sellerComplet = db.prepare(`SELECT nom, type, email FROM sellers WHERE id = ?`).get(auth.id);
  envoyerEmail({
    to: process.env.NOTIFY_EMAIL || "direction@agrisecur.com",
    subject: `Nouveau dossier KYC à examiner — ${sellerComplet.nom}`,
    text:
      `Un vendeur vient de soumettre ses documents KYC sur AgriSecur.\n\n` +
      `Nom / raison sociale : ${sellerComplet.nom}\n` +
      `Type : ${sellerComplet.type}\n` +
      `Email : ${sellerComplet.email}\n` +
      `Document RCCM joint : ${document_rccm_url ? "oui" : "non"}\n\n` +
      `Connectez-vous au back-office (www.agrisecur.com/#admin) pour valider ou rejeter ce dossier.`,
  }).catch((err) => console.error("[mailer] échec de la notification KYC :", err.message));

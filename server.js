...
router.post("/api/admin/sellers/:id/supprimer", (req, res, params) => {
  if (!isAdminAvecLimite(req, res)) return;
  ...
  if (nbCommandes > 0) {
    return send(res, 409, { error: `Suppression impossible : ce vendeur a ${nbCommandes} commande(s) réelle(s) — l'historique financier ne peut pas être effacé, même par un administrateur.` });
  }
  ...
  send(res, 200, { ok: true, supprime: seller.nom });
});

router.post("/api/admin/buyers/:id/supprimer", (req, res, params) => {
  if (!isAdminAvecLimite(req, res)) return;
  ...
  if (nbCommandes > 0) {
    return send(res, 409, { error: `Suppression impossible : cet acheteur a ${nbCommandes} commande(s) réelle(s) — l'historique financier ne peut pas être effacé, même par un administrateur.` });
  }
  const estParrainAvecFilleul = db.prepare(`SELECT COUNT(*) AS n FROM parrainages WHERE parrain_id = ? OR filleul_id = ?`).get(params.id, params.id).n;
  if (estParrainAvecFilleul > 0) {
    return send(res, 409, { error: "Suppression impossible : ce compte est lié à un programme de parrainage déjà récompensé." });
  }
  const aDesFilleuls = db.prepare(`SELECT COUNT(*) AS n FROM buyers WHERE parraine_par = ?`).get(params.id).n;
  if (aDesFilleuls > 0) {
    return send(res, 409, { error: "Suppression impossible : d'autres comptes acheteurs ont été parrainés par celui-ci." });
  }
  ...
  send(res, 200, { ok: true, supprime: buyer.nom });
});

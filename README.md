...
## Signalements — bugs, suggestions et erreurs de connexion en direct

Un bouton **"💬 Signaler un problème"** flotte sur toutes les pages du site,
**y compris l'écran de connexion, avant d'être identifié** — pour qu'un
vendeur ou acheteur bloqué puisse signaler le souci même s'il n'arrive pas
à se connecter.

Chaque signalement capture automatiquement le contexte technique (écran
affiché, rôle éventuel, navigateur) sans effort pour l'utilisateur, et
remonte directement dans un nouvel onglet **"Signalements"** du
back-office, avec statuts (Nouveau / En cours / Résolu) et compteurs.

Le point d'entrée est public (pas de connexion requise, exprès) mais
protégé par le même rate-limiting anti-abus que les autres points sensibles
de l'API.

## PWA — installable comme une vraie application

AgriSecur est maintenant une **Progressive Web App** : installable sur
mobile (Android/iOS) et desktop sans passer par un store, avec icône
dédiée, écran plein sans barre de navigateur, et fonctionnement partiel
hors-ligne pour l'interface (jamais pour les données financières — le
service worker exclut systématiquement `/api/*` du cache, pour ne jamais
afficher un solde ou un statut de commande périmé).

**Sur Android/Chrome** : une bannière "Installer" apparaît automatiquement
et déclenche l'invite native du navigateur.

**Sur iPhone/Safari** (qui ne propose pas d'invite automatique) : la
bannière affiche les étapes manuelles — Partager → "Sur l'écran d'accueil".

**C'est la fondation d'une future app de store**, pas un aboutissement en
soi : le jour où le RCCM est finalisé et qu'une vraie présence App
Store/Play Store devient pertinente, cette même base peut être empaquetée
via **Capacitor** en app native, sans réécrire le frontend.

## Sécurité et fiabilité — deux correctifs suite à un audit

**Clé admin — refus de démarrage si mal configurée.** Si `NODE_ENV=production`
est réglé (à ajouter en plus d'`ADMIN_KEY` sur votre hébergeur) et que la clé
admin est restée à sa valeur par défaut, le serveur refuse de démarrer
plutôt que de tourner avec un accès administrateur ouvert à tous.

**Transactions SQLite sur les opérations financières.** `node:sqlite` n'a pas
de méthode `.transaction()` intégrée (contrairement à `better-sqlite3`) — un
petit utilitaire maison (`executerEnTransaction`, via `BEGIN`/`COMMIT`/
`ROLLBACK`) protège désormais la création de commande (déduction du crédit
de parrainage + insertion + réservation du produit) et les trois points de
clôture (validation acheteur, expiration automatique, médiation de litige)
contre un état incohérent si le serveur s'interrompt en plein milieu d'une
de ces séquences.

## Préparation CinetPay — module de paiement isolé

Un nouveau fichier `payments.js` centralise tout ce qui touchera un jour à
un vrai agrégateur (CinetPay ou autre) — `encaisserPaiement()` à la
création de commande, `declencherReversement()` à la libération des fonds.

**Aujourd'hui : comportement strictement identique à avant** — tout reste
simulé, juste organisé au bon endroit. Chaque commande enregistre déjà une
référence (`reference_paiement_agregateur`, `reference_reversement_agregateur`)
même simulée, pour habituer le schéma de données à ce qui viendra.

**Le jour où CinetPay répond** : ajoutez `CINETPAY_API_KEY`,
`CINETPAY_SITE_ID`, et `PAIEMENT_MODE=reel` comme variables
d'environnement, puis remplissez les deux blocs `TODO` dans `payments.js`
avec les vrais appels API — server.js n'a besoin d'aucune modification.
